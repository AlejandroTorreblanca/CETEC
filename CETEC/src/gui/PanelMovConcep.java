...
public class PanelMovConcep extends JPanel implements ActionListener {
	...
	private JTextField xDescrip;
	private JTextField xConcep;
	private JTextField xNombreTrabajo;
	private JTable tabla;
	private JTextField xCant;
	private ModeloTablaMovimientosCon modelo;
	...
	public PanelMovConcep(VentanaPrincipal w) {
		...
		xDescrip = new JTextField("");
		xDescrip.setEditable(false);
		xDescrip.setMaximumSize(new Dimension(906, 30));
		xDescrip.setMinimumSize(new Dimension(300, 30));
		xCant = new JTextField("");
		fixedSize(xCant, 50, 30);
		xConcep = new JTextField("");
		fixedSize(xConcep, 50, 30);
		...
		modelo = new ModeloTablaMovimientosCon();
		...
		TableColumn columna = tabla.getColumn("Cant.");
	...
	public String buscarConcepto(String codigo) {
		String str = "CONCEPTO='" + codigo + "'";
		try {
	...
	public void actualizarTexto() {
		...
		if (filaSeleccionada != -1) {
			...
			String fecha = modelo.getFechaSeleccionada(filaSeleccionada);
			String nombre = modelo.getDescripSeleccionada(filaSeleccionada);
			String concepto = modelo.getConceptoSeleccionado(filaSeleccionada);
			int cant = modelo.getCantidadSeleccionada(filaSeleccionada);
			SimpleDateFormat d = new SimpleDateFormat("dd/MM/yyyy");
			xMov.setText(numero);
			...
			xConcep.setText(concepto);
			xCant.setText(Integer.toString(cant));
		}
	}
			...
			while (rs.next()) {
				...
				if (concepto != null)
					if (!concepto.isEmpty()) {
						...
						cant = rs.getInt("HORAS");
						String nombre = buscarConcepto(concepto);
						modelo.addFila(Integer.toString(codigo), trabajo, d.format(fecha), concepto, nombre, cant);
					}
			}
	...
	public int guardarCambios() {
		...
		if (!xMov.getText().isEmpty())
			try {
				...
				int precio = buscarPrecio(xConcep.getText());
				if (comprobarDatos(textoOperario.getText(), xTraba.getText(), xConcep.getText(), xFecha.getDate(),
						xCant.getText())) {
					if (rs.first()) { // Update
						if (precio != -1) {
							int importe = Integer.parseInt(xCant.getText()) * precio;
							String str1 = "TRABAJO='" + xTraba.getText() + "',FECHA=? ,OPERARIO='"
									+ textoOperario.getText() + "',CONCEPTO='" + xConcep.getText() + "',HORAS='"
									+ xCant.getText() + "',DESCRIPCION='" + buscarTrabajador(textoOperario.getText())
									+ "',PRECIO='" + Integer.toString(precio) + "',IMPORTE='"
							...
							new PanelMensaje(
									"El usuario introducido no tiene asignado un precio para el año "
						...
						if (precio != -1) {
							int importe = Integer.parseInt(xCant.getText()) * precio;
							String str1 = "(MOVIMIENTO,TRABAJO,FECHA,OPERARIO,CONCEPTO,HORAS,DESCRIPCION,PRECIO,IMPORTE)";
							String str2 = "('" + xMov.getText() + "','" + xTraba.getText() + "',?,'"
									+ textoOperario.getText() + "','" + xConcep.getText() + "','" + xCant.getText()
									+ "','" + textoNombre.getText() + "','" + Integer.toString(precio) + "','"
									+ Integer.toString(importe) + "')";
							if (xFecha.getDate() != null)
								try {
									String fecha = d.format(xFecha.getDate());
	...
	public boolean comprobarDatos(String operario, String trabajo, String concepto, Date fecha, String horas) {
		if (buscarTrabajador(operario).isEmpty()) {
			new PanelMensaje("Operario introducido no encontrado.", "Error en los datos", "error");
			return false;
		}
		if (buscarConcepto(concepto).isEmpty()) {
			new PanelMensaje("Concepto introducido no encontrado.", "Error en los datos", "error");
			return false;
		}
		...
		if (fecha == null) {
			new PanelMensaje("Fecha introducida no válida.", "Error en los datos", "error");
			return false;
		}
		if (horas.isEmpty() || !isNumeric(horas)) {
			new PanelMensaje("Cantidad introducidas no válidas.", "Error en los datos", "error");
			return false;
		}
		return true;
	}
	...
	public int buscarPrecio(String concepto) {
		...
		if (e.getSource() == confirmarButton) {
			...
			if (n != -1 && (!(fechaChooser1.getDate() == null) && !(fechaChooser2.getDate() == null))) {
				if (modelo.getRowCount() > 0) {
					...
					SimpleDateFormat d = new SimpleDateFormat("dd/MM/yyyy");
					vaciarTabla();
					modelo.addFila(xMov.getText(), xTraba.getText(), d.format(xFecha.getDate()), xConcep.getText(),
							xDescrip.getText(), Integer.parseInt(xCant.getText()));
				}
