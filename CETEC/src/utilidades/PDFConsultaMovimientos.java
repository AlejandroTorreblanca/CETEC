	...
	public void createPDF(PanelConsultaMovimientos window) {
		...
		try {
			...
			try {
				...
				new PanelMensaje(
						"Error al crear el pdf, asegurese de que el archivo no está siendo usado por otra aplicación.",
						"Error", "error");
			}
			...
			document.add(table);
			document.add(new Paragraph("\n"));
			Paragraph p = new Paragraph("Número de horas: " + w.getTextoHoras().getText() + "h", subcategoryFont);
